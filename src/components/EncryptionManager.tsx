...
export const EncryptionManager: React.FC<EncryptionManagerProps> = ({ className }) => {
  ...
  const loadEncryptionStats = async () => {
    try {
      ...
      // Get profile encryption stats using secure RPC function
      const { data: profileStatsData, error: profileError } = await supabase.rpc('get_profile_encryption_stats');
      
      if (profileError) throw profileError;
      
      // Parse the JSON response and cast to correct type
      const profileStats = profileStatsData as { encrypted_count: number; total_count: number };
