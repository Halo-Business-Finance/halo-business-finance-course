...
-- Drop all existing function variants first, then recreate

-- Drop all existing functions to avoid signature conflicts
DROP FUNCTION IF EXISTS public.admin_get_profile_summary(uuid);
DROP FUNCTION IF EXISTS public.decrypt_pii_field(text, text);
DROP FUNCTION IF EXISTS public.encrypt_existing_customer_pii();
DROP FUNCTION IF EXISTS public.encrypt_pii_field(text, text);

-- Now recreate them with proper search_path

-- 1. Recreate admin_get_profile_summary function
CREATE FUNCTION public.admin_get_profile_summary(target_user_id uuid)
...
-- 2. Recreate decrypt_pii_field function
CREATE FUNCTION public.decrypt_pii_field(encrypted_data text, field_name text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
-- 3. Recreate encrypt_existing_customer_pii function
CREATE FUNCTION public.encrypt_existing_customer_pii()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
-- 4. Recreate encrypt_pii_field function
CREATE FUNCTION public.encrypt_pii_field(plain_data text, field_name text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
