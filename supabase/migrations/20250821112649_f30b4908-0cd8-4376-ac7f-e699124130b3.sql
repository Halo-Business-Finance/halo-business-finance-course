-- Enhanced Security Monitoring and Audit Logging (Fixed)
-- This migration implements comprehensive security fixes from the security review

-- 1. Create a function to log profile data access by admins (UPDATE/INSERT/DELETE only)
...
-- 2. Create trigger for profile modification logging (not SELECT, only modifications)
...
-- 6. Enhanced safe_profiles view with proper security
...
WHERE 
  ...
  p.user_id = auth.uid()
  OR 
  -- Admins can see all profiles (modifications will be logged via trigger)
  is_admin(auth.uid());

-- 7. Add additional security event logging for critical operations
...
-- 8. Create a function to manually log profile view access (since we can't use SELECT triggers)
