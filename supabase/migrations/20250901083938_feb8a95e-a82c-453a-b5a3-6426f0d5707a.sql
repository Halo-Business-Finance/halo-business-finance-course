...
CREATE TABLE IF NOT EXISTS public.module_quizzes (
  ...
  UNIQUE(adaptive_module_id, question_number)
);
...
CREATE TABLE IF NOT EXISTS public.module_quiz_attempts (
  ...
  adaptive_module_instance_id uuid REFERENCES public.adaptive_module_instances(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_module_quizzes_adaptive_module ON public.module_quizzes(adaptive_module_id);
CREATE INDEX IF NOT EXISTS idx_module_quiz_attempts_user_module ON public.module_quiz_attempts(user_id, adaptive_module_instance_id);
...
BEGIN
  RETURN EXISTS (
    ...
    AND adaptive_module_instance_id = p_module_instance_id 
...
BEGIN
  RETURN EXISTS (
    ...
    JOIN public.adaptive_module_instances ami ON ami.id = mqa.adaptive_module_instance_id
