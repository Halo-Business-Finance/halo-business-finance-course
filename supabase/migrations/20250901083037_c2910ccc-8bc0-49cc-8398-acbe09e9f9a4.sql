-- Create adaptive learning tables if they don't exist
-- This migration handles existing tables/policies gracefully

-- Create adaptive_modules table if it doesn't exist
...
DROP POLICY IF EXISTS "Admins can manage adaptive modules" ON public.adaptive_modules;
DROP POLICY IF EXISTS "Enrolled users can view adaptive modules" ON public.adaptive_modules;
...
-- Insert the 7 adaptive modules (using ON CONFLICT to avoid duplicates)
...
-- Create adaptive_module_instances table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.adaptive_module_instances (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  UNIQUE(user_id, course_id, adaptive_module_id)
);
...
DROP POLICY IF EXISTS "Users can manage their own adaptive module instances" ON public.adaptive_module_instances;
DROP POLICY IF EXISTS "Admins can view all adaptive module instances" ON public.adaptive_module_instances;
