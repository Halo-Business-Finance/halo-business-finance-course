-- CRITICAL SECURITY REMEDIATION: Enhanced MFA/Biometric Security (with CASCADE)

-- Drop existing functions with CASCADE to remove dependent policies
DROP FUNCTION IF EXISTS public.validate_mfa_access(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.validate_biometric_access(uuid) CASCADE;
...
-- Recreate the enhanced RLS policies for MFA and biometric tables
...
CREATE POLICY "Enhanced secure biometric access" 
