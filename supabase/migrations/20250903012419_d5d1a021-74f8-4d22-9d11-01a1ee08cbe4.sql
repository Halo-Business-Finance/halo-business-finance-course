...
-- First, update any existing 'intermediate' skill levels to 'expert' 
UPDATE course_modules 
SET skill_level = 'expert'::skill_level
WHERE skill_level = 'intermediate'::skill_level;

UPDATE adaptive_module_instances 
SET current_difficulty_level = 'expert'
WHERE current_difficulty_level = 'intermediate';

-- Update any other tables that might have intermediate references
UPDATE adaptive_modules 
SET difficulty_level = 'expert'
WHERE difficulty_level = 'intermediate';

-- Drop and recreate the skill_level enum to only include beginner and expert
